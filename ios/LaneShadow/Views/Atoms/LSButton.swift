...
private struct LSButtonLabel: View {
    ...
    var body: some View {
        HStack(spacing: LSButtonStyle.labelSpacing(in: theme)) {
            if let leadingIcon {
                LSIcon(name: leadingIcon, size: .sm, color: iconColor)
            }

            LSText(title, variant: .label.md, color: textColor)

            if let trailingIcon {
                LSIcon(name: trailingIcon, size: .sm, color: iconColor)
            }
        }
    ...
    private var textColor: ContentColor {
        switch variant {
        case .primary, .accept, .destructive:
            .onSignal
    ...
    private var iconColor: IconContentColor {
        switch variant {
        case .primary, .accept, .destructive:
            .onSignal
        case .secondary, .ghost, .outline:
