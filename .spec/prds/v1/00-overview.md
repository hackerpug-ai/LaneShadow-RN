...
1. **Conversational Planning on the Map**: A rider opens the app and sees a map with a chat input bar at the bottom. They type what they want — the AI interprets intent, generates 2–3 scenic route alternatives, and renders them directly on the map as chat attachments. The rider can then refine iteratively: "avoid Highway 1", "make it shorter", "add a stop at Big Sur." Each exchange builds on the last within a persistent session. The AI's responses appear as temporary overlays on the map that fade away, keeping the map primary. Full chat history is available by expanding the chat view.
...
|------------------|-----------|--------|
| NLP (Conversational Planning) | 0% (0/11 UCs) | Chat infrastructure does not exist; current implementation is manual form-based |
...
**Critical Architectural Gap**: The V1 PRD describes a **conversational AI copilot** for ride planning. The current codebase implements a **manual planning form**. This is a fundamental architectural mismatch requiring chat infrastructure to be built from scratch.
