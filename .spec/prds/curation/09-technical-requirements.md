...
| ID | Rule | Enforcement |
|----|------|-------------|
| **P0** | **No on-device LLM.** All LLM inference is server-side Haiku. | App bundle contains zero model files; no `mlx-local`, no Core ML `.mlpackage`, no ONNX, no `transformers`. CI bundle-size audit flags any >50MB binary asset. |
...
| **P4** | All LLM extraction runs at **`temperature=0`** with retry-on-degeneration | Python/Node wrapper enforces temp=0 for all Instructor/Anthropic calls. |
...
| **Intent Query Service** | Orchestrator for natural language search. Flow: (1) normalize user intent string and look up in Intent Cache; (2) cache hit → validated params returned in <5ms; (3) cache miss + online → Haiku extracts params via Instructor with the 10-key schema, enum-validates, caches the result; (4) cache miss + offline → returns `OFFLINE_UNSUPPORTED` so UI can show the "connect to search" empty state with recent-intents shortcuts. Validated params always pass through deterministic `params_to_sql()` → op-sqlite → top 10 routes. **No on-device LLM is invoked at any step.** No LLM ever sees route candidates (P1). Validated: Haiku 100% pass / ~1.0s extraction (2026-04-10) |
...
1. **Lean tier** (`curated_routes`) — pre-digested, query-optimized. Synced to device. Fields are sized for SQL filtering and fast card rendering. No LLM ever reads these rows — Haiku's only input is the user's raw intent string, and ranking is pure SQL `ORDER BY`.
...
Contains only what's needed for (a) SQL discovery queries and (b) UI card rendering. No LLM ever sees these rows.
...
**Size target:** ~200–300 bytes per row on disk. Lean fields support SQL filtering (indexed columns) and card rendering. Haiku (server-side, cache-miss only) receives only the user's intent string (~10–20 tokens), never route cards.
...
1. Set up Instructor + Anthropic SDK (temperature=0 enforced)
2. Define Pydantic schema for RouteAttributes, versioned via `extractionSchemaVersion`
3. Implement parallel extraction with ThreadPoolExecutor
4. Add error handling, retries, and retry-on-degeneration guard
5. **Calibration gate** — fit composite-score weights against Rider Magazine Top 50 + FHWA ground truth; emit fit report; check top-10 recovery rate ≥ target threshold before proceeding
6. Run extraction over full scraped corpus (gated on calibration pass)
7. Validate extraction quality on a held-out slice of ground truth
...
7. Add offline support (lean-tier sync, cached enrichment replay)
8. **Intent search (UC-DISC-07) — Haiku + cache path:**
   - Port the validated intent-extraction prompt + Pydantic schema from research into `/api/intent/extract-params`
   - Implement `intent_param_cache` table in op-sqlite
   - Port `params_to_sql()` + enum validator to TypeScript (shared between cache-hit and cache-miss paths)
   - Wire Intent Query Service: normalize → cache lookup → Haiku (online) → cache write → SQL → render
   - Offline cache-miss UX: "connect to search" empty state with recent-intents shortcut chips
