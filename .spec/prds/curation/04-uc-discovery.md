...
| UC-DISC-07 | Intent-Based Search | User types or speaks natural language intent; on-device Qwen3.5 0.8B extracts query params; deterministic SQL returns pre-selected matching routes |
...
**Description:** Rider types or speaks a natural language intent ("twisty mountain roads near me", "something remote and challenging"). On-device Qwen3.5 0.8B extracts structured query parameters via slot-filling. Deterministic `params_to_sql()` converts these params into a SQL query that runs against local `discovery.db`, returning pre-selected routes ordered by pre-computed scores. The local model never sees or ranks route candidates — selection is entirely the database's job.

**Validated:** 2026-04-10 test suite — 93% scenario pass rate, 0.84 avg F1, 0.89 Jaccard agreement with Haiku, 100% valid JSON, ~1.5s latency. See `.spec/research/local-models/INTENT_TO_QUERY_RESULTS_2026-04-10.md`.
...
- ☐ On-device Qwen3.5 0.8B processes input locally with no network request
- ☐ Loading indicator displays during Qwen inference (~1.5s)
...
- ☐ If local query returns zero results AND device is online: escalate to Haiku for param clarification, re-run local query
- ☐ If local query returns zero results AND device is offline: show empty state with retry suggestion
- ☐ "Clear search" resets to proximity-based browse (UC-DISC-01)
- ☐ System works fully offline for non-zero result intents
