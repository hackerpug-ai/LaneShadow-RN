...
Frontload the repository restructure so backend work can move to `convex/`, the Expo app can move to `react-native/`, and downstream native-rewrite work can proceed against the intended multi-app layout.

## Human Testing Gate

**Gate:** The repo supports `convex/` as the backend root and `react-native/` as the Expo app root, with root workflows and docs updated to match.
...
2. Confirm all Convex source, config, and generated files live under `convex/` as the authoritative backend location.
...
| RESTR-002 | Move `convex/` to `convex/` | convex-implementer | 300 min |
