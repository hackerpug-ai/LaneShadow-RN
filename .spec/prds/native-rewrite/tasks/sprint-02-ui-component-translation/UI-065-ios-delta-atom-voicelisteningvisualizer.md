# UI-065: iOS delta atom — `VoiceListeningVisualizer` (audio amplitude waveform atom for UC-VOICE-02); SwiftUI Canvas + TimelineView
...
Sprint 2 translates the React Native baseline into native platform components and sandbox scenarios before rider-facing feature sprints resume. This task covers the `delta-atom` slice for `iOS delta atom — VoiceListeningVisualizer (audio amplitude waveform atom for UC-VOICE-02); SwiftUI Canvas + TimelineView`.

**Objective:** Implement iOS delta atom — `VoiceListeningVisualizer` (audio amplitude waveform atom for UC-VOICE-02); SwiftUI Canvas + TimelineView as SwiftUI work with parity to the RN baseline, token-only styling, and sandbox coverage.
...
### MUST
- Implement the listed components or compositions with parity-aligned naming and interfaces: iOS delta atom — `VoiceListeningVisualizer` (audio amplitude waveform atom for UC-VOICE-02); SwiftUI Canvas + TimelineView.
...
**THEN** The listed components or compositions exist in the correct iOS SwiftUI directories with parity-aligned names and signatures: iOS delta atom — `VoiceListeningVisualizer` (audio amplitude waveform atom for UC-VOICE-02); SwiftUI Canvas + TimelineView.
**Verify:** `printf "%s\n" "iOS delta atom — `VoiceListeningVisualizer` (audio amplitude waveform atom for UC-VOICE-02); SwiftUI Canvas + TimelineView"`
...
|---|---|---|---|
| TC-1 | AC-1 | The listed components or compositions exist in the correct iOS SwiftUI directories with parity-aligned names and signatures: iOS delta atom — `VoiceListeningVisualizer` (audio amplitude waveform atom for UC-VOICE-02); SwiftUI Canvas + TimelineView. | `printf "%s\n" "iOS delta atom — `VoiceListeningVisualizer` (audio amplitude waveform atom for UC-VOICE-02); SwiftUI Canvas + TimelineView"` |
...
|---|---|---|---|---|
| VoiceListeningVisualizer | **RN baseline pending — properties derived from task spec and UC-VOICE-02 voice interaction UX** | n/a (NEW component — delta) | `ios/LaneShadow/Views/Atoms/VoiceListeningVisualizer.swift` | 1 variant (audio amplitude waveform) × 2 states (idle/recording) × animation (amplitude bars driven by audio input) |
...
> **Note**: This is a **NEW delta component** — no RN baseline exists. Properties are derived from the task description ("audio amplitude waveform atom for UC-VOICE-02; SwiftUI Canvas + TimelineView") and the voice interaction UX specification which shows a waveform visualization pattern: `●  ● ● ● ●  ●` with animated amplitude bars during recording.
...
| Property | Source | Value | Android | iOS | Token |
|---|---|---|---|---|---|---|
| animationType | Task spec | `TimelineView` + `Canvas` drawing | `animateFloatAsState(targetValue = amplitude)` | `TimelineView(.animation(minimumInterval: 0.1))` + Canvas draw | n/a |
| duration | Task spec | `100ms` (fast response to audio) | `durationMillis = 100` | `0.1` (minimumInterval) | ESCALATE — propose `motion.duration.fast = 100` |
