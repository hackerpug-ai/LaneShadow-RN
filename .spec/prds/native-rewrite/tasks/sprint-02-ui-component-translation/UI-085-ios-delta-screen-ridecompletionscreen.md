# UI-085: iOS delta screen — `RideCompletionScreen` (same contract)
...
Sprint 2 translates the React Native baseline into native platform components and sandbox scenarios before rider-facing feature sprints resume. This task covers the `delta-screen` slice for `iOS delta screen — RideCompletionScreen (same contract)`.

**Objective:** Implement iOS delta screen — `RideCompletionScreen` (same contract) as SwiftUI work with parity to the RN baseline, token-only styling, and sandbox coverage.
...
### MUST
- Implement the listed components or compositions with parity-aligned naming and interfaces: iOS delta screen — `RideCompletionScreen` (same contract).
...
**THEN** The listed components or compositions exist in the correct iOS SwiftUI directories with parity-aligned names and signatures: iOS delta screen — `RideCompletionScreen` (same contract).
**Verify:** `printf "%s\n" "iOS delta screen — `RideCompletionScreen` (same contract)"`
...
|---|---|---|---|
| TC-1 | AC-1 | The listed components or compositions exist in the correct iOS SwiftUI directories with parity-aligned names and signatures: iOS delta screen — `RideCompletionScreen` (same contract). | `printf "%s\n" "iOS delta screen — `RideCompletionScreen` (same contract)"` |
