# UI-082: Android delta organism — `GatekeeperUpgradePrompt` (paywall modal with tier cards + benefits + CTAs + offline variant for UC-GATE-03 + UC-GATE-08); consumes `BottomSheetWrapper`, `Card`, `Badge`, `Button`
...
Sprint 2 translates the React Native baseline into native platform components and sandbox scenarios before rider-facing feature sprints resume. This task covers the `delta-organism` slice for `Android delta organism — GatekeeperUpgradePrompt (paywall modal with tier cards + benefits + CTAs + offline variant for UC-GATE-03 + UC-GATE-08); consumes BottomSheetWrapper, Card, Badge, Button`.

**Objective:** Implement Android delta organism — `GatekeeperUpgradePrompt` (paywall modal with tier cards + benefits + CTAs + offline variant for UC-GATE-03 + UC-GATE-08); consumes `BottomSheetWrapper`, `Card`, `Badge`, `Button` as Jetpack Compose work with parity to the RN baseline, token-only styling, and sandbox coverage.
...
### MUST
- Implement the listed components or compositions with parity-aligned naming and interfaces: Android delta organism — `GatekeeperUpgradePrompt` (paywall modal with tier cards + benefits + CTAs + offline variant for UC-GATE-03 + UC-GATE-08); consumes `BottomSheetWrapper`, `Card`, `Badge`, `Button`.
...
**Verify:** `printf "%s\n" "Android delta organism — `GatekeeperUpgradePrompt` (paywall modal with tier cards + benefits + CTAs + offline variant for UC-GATE-03 + UC-GATE-08); consumes `BottomSheetWrapper`, `Card`, `Badge`, `Button`"`
...
|---|---|---|---|---|
| GatekeeperUpgradePrompt | **RN BASELINE PENDING** — new delta organism for UC-GATE-01 | `node_modules/@gorhom/bottom-sheet/src/components/bottomSheetModal/BottomSheetModal.tsx` (reference for modal patterns); `node_modules/react-native/Libraries/Components/Modal/Modal.js` | `android/app/src/main/java/com/laneshadow/ui/organisms/GatekeeperUpgradePrompt.kt` | 1 fixed layout × 2 states (visible/dismissed) × 3 prompt types (limit/renewal/feature) |
