# UI-084: Android delta screen — `RideCompletionScreen` (full completion flow composing `CompletionSummaryCard` + `RideShareSheet` for UC-NAV-06 + UC-FLOW-08)
...
Sprint 2 translates the React Native baseline into native platform components and sandbox scenarios before rider-facing feature sprints resume. This task covers the `delta-screen` slice for `Android delta screen — RideCompletionScreen (full completion flow composing CompletionSummaryCard + RideShareSheet for UC-NAV-06 + UC-FLOW-08)`.

**Objective:** Implement Android delta screen — `RideCompletionScreen` (full completion flow composing `CompletionSummaryCard` + `RideShareSheet` for UC-NAV-06 + UC-FLOW-08) as Jetpack Compose work with parity to the RN baseline, token-only styling, and sandbox coverage.
...
### MUST
- Implement the listed components or compositions with parity-aligned naming and interfaces: Android delta screen — `RideCompletionScreen` (full completion flow composing `CompletionSummaryCard` + `RideShareSheet` for UC-NAV-06 + UC-FLOW-08).
...
**THEN** The listed components or compositions exist in the correct Android Compose directories with parity-aligned names and signatures: Android delta screen — `RideCompletionScreen` (full completion flow composing `CompletionSummaryCard` + `RideShareSheet` for UC-NAV-06 + UC-FLOW-08).
**Verify:** `printf "%s\n" "Android delta screen — `RideCompletionScreen` (full completion flow composing `CompletionSummaryCard` + `RideShareSheet` for UC-NAV-06 + UC-FLOW-08)"`
...
|---|---|---|---|
| TC-1 | AC-1 | The listed components or compositions exist in the correct Android Compose directories with parity-aligned names and signatures: Android delta screen — `RideCompletionScreen` (full completion flow composing `CompletionSummaryCard` + `RideShareSheet` for UC-NAV-06 + UC-FLOW-08). | `printf "%s\n" "Android delta screen — `RideCompletionScreen` (full completion flow composing `CompletionSummaryCard` + `RideShareSheet` for UC-NAV-06 + UC-FLOW-08)"` |
...
|---|---|---|---|---|
| RideCompletionScreen | **RN BASELINE PENDING** — new delta screen for UC-FLOW-08 | Composes: `CompletionSummaryCard` (UI-078), `RideShareSheet` (UI-080); reference: `node_modules/react-navigation/src/native navigators/StackNavigator.tsx`; `node_modules/react-native/Libraries/Components/View/View.js` | `android/app/src/main/java/com/laneshadow/ui/screens/RideCompletionScreen.kt` | 1 fixed layout × 3 states (loading/loaded/error) × 2 action modes (save/discard) |
...
**Source files read:**
- Task specification: UI-084 task description
- Composed screens: CompletionSummaryCard (UI-078), RideShareSheet (UI-080)
