# UI-004: Bootstrap iOS sandbox host, catalog navigation, and RN reference registry
...
Sprint 2 translates the React Native baseline into native platform components and sandbox scenarios before rider-facing feature sprints resume. This task covers the `sandbox` slice for `Bootstrap iOS sandbox host, catalog navigation, and RN reference registry`.

**Objective:** Implement Bootstrap iOS sandbox host, catalog navigation, and RN reference registry as SwiftUI work with parity to the RN baseline, token-only styling, and sandbox coverage.
...
### MUST
- Implement the listed components or compositions with parity-aligned naming and interfaces: Bootstrap iOS sandbox host, catalog navigation, and RN reference registry.
- Use only the LaneShadow core theme contract locked by `UI-001`: `tokens/semantic/semantic.tokens.json` as the source of truth, generated `native-theme` outputs as the shared packaging layer, and the platform theme entry points (`LaneShadowTheme` on Android, `.laneShadowTheme()` on iOS) for colors, spacing, typography, radii, elevation, and state styling.
- Register sandbox scenarios for default, interactive, and edge-case states with RN reference labels.
- Support open-by-ID scenario navigation and RN reference registry consumption in the sandbox host.
...
- ios/LaneShadow/App.swift — SandboxLaunch.configure(...) + DEBUG root-view gating (installed)
- ios/LaneShadow/Sandbox/LaneShadowSandboxEntry.swift — SandboxRoot entry wiring previewWrapper theme (installed)
- ios/LaneShadow/Sandbox/LaneShadowStories.swift — @MainActor aggregator (installed)
- ios/LaneShadow.xcodeproj/project.pbxproj — NativeSandbox SPM local package at relativePath `../../native-sandbox/ios` (installed)

## ACCEPTANCE CRITERIA

### AC-1
**GIVEN** native-sandbox is installed (SandboxLaunch, SandboxRoot, LaneShadowStories aggregator, NativeSandbox SPM local package wired in project.pbxproj).
**WHEN** The reviewer runs `make ios_sandbox` (which passes `-LaneShadowSandbox` to the app on launch).
**THEN** The app boots into SandboxRoot, `HelloStory` (seed story registered in `LaneShadowStories.all`) renders under `themedPreview { $0.laneShadowTheme() }`, and the story-tree drawer opens with the tier/component hierarchy.
**Verify:** `make ios_sandbox` (success = HelloStory renders; drawer enumerates LaneShadowStories.all)
...
|---|---|---|---|
| TC-1 | AC-1 | The listed components or compositions exist in the correct iOS SwiftUI directories with parity-aligned names and signatures: Bootstrap iOS sandbox host, catalog navigation, and RN reference registry. | `printf "%s\n" "Bootstrap iOS sandbox host, catalog navigation, and RN reference registry"` |
...
**Pattern:** NavigationStack or enum-backed catalog routing plus fixture-driven scenario registry and RN reference labels.
