...
- `ChatInput` (organism) — always-visible input bar with suggestions and send/cancel state
- `ChatTranscript` (organism) — renders message list with reverse-chronological scrolling
- `SuggestionChips` (molecule) — prompt suggestions shown in empty state
- `KeyboardAvoidingInput` (molecule) — ensures input bar stays above keyboard
- `EmptyState` (molecule) — empty chat placeholder with headline and body
- `MenuLayout` (template) — hosts the chat screen within the app shell
- `AppHeader` (molecule) — screen header with title and optional action
...
**UI Components (from Sprint 2):**
- `ChatInput` (organism) — input field, send/cancel button, disabled state while sending
- `ChatTranscript` (organism) — host list that receives the optimistic message
- `IconSymbol` (atom) — send and cancel (×) icons inside the input
- `Button` (atom) — underlying send/cancel action element
- `KeyboardAvoidingInput` (molecule) — keeps input visible during send
- `SuggestionChips` (molecule) — cleared/re-shown around send state

**New Compositions Needed:**
- `OptimisticMessageBubble` (proposed molecule) — temp-ID message bubble that visually distinguishes pending vs. confirmed state; no existing atomic-catalog component renders a user message bubble with pending styling
...
**UI Components (from Sprint 2):**
- `TypingIndicator` (atom) — 3 animated dots while `status: 'running'`
- `ChatTranscript` (organism) — host for the streaming assistant message
- `MarkdownText` (molecule) — incremental markdown rendering (headers, lists, links, code)
- `ReasoningCard` (molecule) — optional reasoning surface during agent think
- `ThinkingCard` (molecule) — stepwise progress rendering mid-stream
- `ChatInput` (organism) — reflects sending/streaming state
...
**UI Components (from Sprint 2):**
- `RoutingCard` (molecule) — live planning progress card with phase pills
- `RouteAttachmentCard` (molecule) — completed route option card rendered inline
- `RouteMiniMap` (molecule) — overview geometry preview per route
- `PlanningCard` (molecule) — progress + message composite during planning
- `WeatherPill` (molecule) — rain/wind/temperature overlays on the card
- `RainBadge` (molecule) — rain warning indicator
- `WindBadge` (molecule) — wind warning indicator
- `StatRow` (molecule) — distance and duration stats
- `ChatTranscript` (organism) — host for the inline card attachments
- `Button` (atom) — "Try again" retry on failure state
...
**UI Components (from Sprint 2):**
- `RouteAttachmentCard` (molecule) — selectable route card with highlighted border
- `IconSymbol` (atom) — checkmark icon on the selected card
- `Button` (atom) — "View on Map" and "Start Planning" CTAs
- `RouteMiniMap` (molecule) — preview of the selected route
- `MapViewWrapper` (organism) — target map surface for camera fit
- `RoutePolyline` (atom) — rendered polyline for the selected route on map
- `RouteDetailsSheet` (organism) — bottom sheet showing route label and actions
...
**UI Components (from Sprint 2):**
- `ChatTranscript` (organism) — paginated message list with reverse layout
- `SessionSidebar` (organism) — list of previous sessions
- `SessionCard` (molecule) — per-session summary entry
- `SearchBar` (molecule) — search input for filtering messages
- `Skeleton` (atom) — loading indicator while older messages load
- `AppHeader` (molecule) — toolbar hosting the search icon
- `IconSymbol` (atom) — search icon affordance

**New Compositions Needed:**
- `ChatSearchResultItem` (proposed molecule) — result row with highlighted query match and jump-to-message behavior; no atomic-catalog component currently renders search results with query highlighting
...
**UI Components (from Sprint 2):**
- `BottomActionSheet` (template) — "Current Location" / "Select on Map" options sheet
- `LocationSearchCard` (molecule) — inline chat card rendering attached location
- `LocationInput` (molecule) — address entry / selection control
- `MapViewWrapper` (organism) — full-screen map for tap-to-select
- `WaypointMarker` (molecule) — pin at tapped coordinates
- `Button` (atom) — "Attach Location" and "View on Map" CTAs
- `IconSymbol` (atom) — location option icons
- `ChatInput` (organism) — attachment button trigger
...
**UI Components (from Sprint 2):**
- `ErrorMessage` (molecule) — inline error surface above the input
- `ConnectionBanner` (molecule) — network-loss banner
- `ErrorToast` (molecule) — transient error notification
- `Button` (atom) — "Retry" and "Upgrade Plan" CTAs
- `SuggestionChips` (molecule) — "Try asking differently" rephrase suggestions
- `ChatInput` (organism) — preserves message text and sending state for retry
- `IconSymbol` (atom) — warning/error glyphs
