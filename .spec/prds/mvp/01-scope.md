...
- **Rich enrichment → Butler-grade "why this road is good"** — the `curated_route_enrichments` table is verified EMPTY (0 docs); detail uses lean data only for MVP. Post-MVP this becomes the rider-validated **"why this road is good" enrichment** opportunity (riders pay for and revere Butler's *graded* roads): AI vision + LLM over geometry/elevation/POIs to generate the headline + the "why." See [11-post-mvp-opportunities.md → O1](./11-post-mvp-opportunities.md).
- **Waypoints / "Moments Near Me"** — strategy Phase 0.5; routes alone satisfy the core job.
- **Weather intelligence → weather-window ("is it worth riding this weekend, and where")** — strategy Phase 2; MVP shows only basic current conditions. Post-MVP this is the rider-validated **weather-window intelligence** opportunity (recurring personal-threshold go/no-go — *"30% rain is my threshold," "<41°F I park it"*) and the clearest competitive moat. See [11-post-mvp-opportunities.md → O2](./11-post-mvp-opportunities.md).
- **Ride/loop composition** — composing curated roads + waypoint stops into a timed loop ("3-hour scenic loop from home with a coffee stop"). Out of MVP: the MVP plots single curated routes. Note basic algorithmic loop-gen is already commoditized (Calimoto/Scenic/Kurviger); the *curated* version is the post-MVP opportunity. See [11-post-mvp-opportunities.md → O3](./11-post-mvp-opportunities.md).
- **Personalized ranking** — learning from saves/ratings/"rode-it" to surface "roads like the ones you loved." Out of MVP: no usage data exists pre-launch. Post-MVP retention layer. See [11-post-mvp-opportunities.md → O4](./11-post-mvp-opportunities.md).
