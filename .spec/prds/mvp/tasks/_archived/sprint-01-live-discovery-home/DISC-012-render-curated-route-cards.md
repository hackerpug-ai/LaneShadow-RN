...
**Write-prohibited:** components/chat/route-attachment-card.tsx — coupled to PlannedRouteOptionsView; do NOT fork its planned-route contract. If reused, reuse as-is. · hooks/use-active-session-route.ts — the resolution machinery is reused unchanged. · convex/** — DATA-008 supplies the routing_card route_options; backend is locked. · Any file not explicitly listed above
