...
- `server/convex/actions/agent/agents/routingAgent.ts:660-837,866-977,1074-1122` — **PRIMARY PATTERN** runPlanRoute: createForAgentInternal → build PlannedRouteOptionsView → updatePlanStatus(completed,result) → return `{type:'routes',data,routePlanId}`; tool registration + dispatch + result extraction. Mirror for curated.
- `server/convex/actions/agent/sendMessage.ts:25-29,226-291` — TOOL_TO_CARD_KIND map (register discoverCuratedRoutes) + onToolFinish (routePlanId → kind:'routing_card' session_messages row with route_options attachment).
- `server/convex/actions/agent/agents/orchestrator.ts:62-107,160-265,300-344` — add discovery_agent meta-tool: determineAvailableTools, the sub-agent Tool, executeOrchestratorTool case, extractOrchestratorAttachments.
- `server/convex/curatedRoutes.ts:20-54,117-282` — listCuratedRoutes args/returns + buildRouteCard/norm (consumed read-only; reuse the 0–1 norm convention; archetype-set + state-variant filtering already handled).
- `server/convex/actions/agent/planRide.ts:28-73,104-155` — plannedRouteOptionsViewValidator + buildOptionsFromResults — the exact route_options shape to conform to.

## Guardrails

**Write-allowed:** `server/convex/actions/agent/tools/discoverCuratedRoutes.ts` (NEW) · `server/convex/actions/agent/agents/discoveryAgent.ts` (NEW) · `server/convex/actions/agent/agents/orchestrator.ts` (MODIFY: register meta-tool + attachment extraction only) · `server/convex/actions/agent/sendMessage.ts` (MODIFY: add to TOOL_TO_CARD_KIND only) · `server/convex/actions/agent/agents/types.ts` (MODIFY: add result type only) · `server/convex/actions/agent/tools/__tests__/discoverCuratedRoutes.integration.test.ts` (NEW) · `server/convex/actions/agent/agents/__tests__/orchestrator.discovery.test.ts` (NEW)

**Write-prohibited:** `server/convex/curatedRoutes.ts` (contract frozen) · `server/convex/schema.ts` + `server/models/**` (no schema change) · `server/convex/db/routePlans.ts` (consume as-is) · `server/convex/util/archetypeMap.ts` (consume as-is) · any `app/**`/`hooks/**`/`components/**` (DISC-012 owns the client render) · any file not listed.

## Verification gates

1. `pnpm test -- server/convex/actions/agent/tools/__tests__/discoverCuratedRoutes.integration.test.ts` — all AC scenarios green; PRIMARY AC-1 watched RED against an empty catalog before GREEN.
2. `pnpm test -- server/convex/actions/agent/agents/__tests__/orchestrator.discovery.test.ts` — discovery_agent attachment extraction.
