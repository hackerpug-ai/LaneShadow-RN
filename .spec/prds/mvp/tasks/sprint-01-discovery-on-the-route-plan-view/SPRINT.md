...
## Remedial Round 4 — Red-Hat Cycle 1 Re-Review Findings (cycle 2)

**Source:** [red-hat-sprint-01-discovery-2026-07-03T18-53-01Z.md](../../../reviews/red-hat-sprint-01-discovery-2026-07-03T18-53-01Z.md) (cycle 1 re-review). All 8 original findings RESOLVED. These tasks close the 3 new HIGH findings + the elevated M-1 (same anti-pattern as the original CRITICAL).

| ID | Severity | Title | Agent | Estimate |
|----|----------|-------|-------|----------|
| REDHAT-FIX-004 | HIGH | **H-1/H-2:** extract `MOCK_SEMANTIC` to a shared RN test fixture (`react-native/test-helpers/mock-semantic.ts`) AND create a shared `setupHomeScreenMocks()` / `renderHomeScreen()` helper; update all 4 RN integration test files to import from shared helpers instead of duplicating ~207 lines + ~60 `vi.mock()` blocks each | react-native-ui-implementer | 120 min |
| REDHAT-FIX-005 | HIGH | **M-1 elevated + H-3 + M-2 + M-3:** (1) guard `durationSeconds` and `legsCount` at `discoverCuratedRoutes.ts:155-156` with the same `!= null` pattern as `distanceMeters` (M-1 — same anti-pattern as original CRITICAL); (2) improve state write-back purity test at `listCuratedRoutes.state.integration.test.ts:63-95` — add a comment acknowledging the idempotent-normalization limitation and noting Convex query guarantee is the real guard, OR sample raw `curated_routes` rows via an internal query that bypasses `buildRouteCard` (H-3); (3) add `afterEach` cleanup for `route_plans` rows created by `discoverCuratedRoutes.scores.integration.test.ts` (M-2); (4) guard `centerPoint` fallback at `discoverCuratedRoutes.ts:120` to avoid `{lat:0,lng:0}` for best-sort (M-3) | convex-implementer | 120 min |

**REDHAT-FIX cycle 2 task detail files** (expanded by /but-sprint-tasks-plan --only):

- REDHAT-FIX-004-rn-shared-test-helpers-mock-semantic-extraction.md
- REDHAT-FIX-005-convex-fabricated-zeros-state-purity-cleanup.md
