...
- **Reality-Gate flow registry (applied 2026-06-23 by `/kb-e2e-retrofit --apply`).** The PRD now has a complete core+edge flow registry at `.spec/scenarios/<UC>/*.scenario.md` for all 17 UCs (`flow_coverage_check.py --prd` → exit 0). Sprint 01 has a locked `human-flows.json` at `tasks/sprint-01-discovery-on-the-route-plan-view/human-flows.json` (16 flows covering 8 in-scope UCs; `flow_coverage_check.py --sprint` → exit 0); the 12 PRIMARY ACs in Sprint 01's flow-owning task files now carry a `flow_ref:` line bound to the matching `HF-*` flow id. **Closing Sprint 01** requires `/kb-run-sprint tasks/sprint-01-discovery-on-the-route-plan-view` to replay every locked flow at the human surface (cold-boot Maestro `discovery-full-gate.yaml` + live-Convex integration tests) — the gate computes the verdict from real exit codes + artifacts, not agent prose. **Sprint 02 and Sprint 03** remain PLANNED and will have their `human-flows.json` JIT-derived from this same registry by `/kb-sprint-tasks-plan` when they activate (Sprint 02 scope: `UC-DATA-03,UC-DATA-06,UC-DTL-01..04,UC-SAVE-01..02`; Sprint 03 scope: `UC-DISC-01` journey replay post-integration). See [`RETROFIT-AUDIT.md`](./RETROFIT-AUDIT.md) for the full audit + remediation record.
