# Sprint 10: Community Sources — Matching, Reconciliation & Signal Merge
...
Sprint 9 writes raw `PostExtraction` artifacts to the `route_posts_raw` Convex table — one Claude Haiku 4.5 call per community post producing structured identifier mentions, sentiment, aspect scores, attributes, and warnings. **This sprint reads those artifacts**, runs semantic matching via `findCandidateRoutesByEmbedding` + LLM rerank to decide which curated route(s) each post mentions, writes `route_matches` audit rows via `addRouteMatch`, and runs LLM reconciliation on routes with multiple conflicting mentions.
...
These feed Sprint 8's composite scoring. **All custom NLP (sentiment classifier, aspect scorer, attribute detector, surface-type extractor) is gone** — it's all inside the single Claude call in Sprint 9's `PostExtraction` schema. This sprint is purely matching, reconciliation, and signal merge.
...
3. **Signal merge** — reconciled signals feed `mentionFrequencyScore` and aspect aggregations on `curated_routes`, which Sprint 8 composite scoring consumes.
...
7. **Run Sprint 8 composite scoring** — Re-run scoring. Verify the `mentionFrequencyScore` weight now has real data and that routes with high community engagement rise accordingly.
...
- [ ] Composite scoring (Sprint 8) consumes the new signals
...
|----|-------|------|-------|----------|--------|----------|------------|--------|------|
| RID-003 | Community Post Matching — Semantic Search + LLM Rerank | FEATURE | python-implement | P0 | L | 360 | Epic 3 (INF-003, INF-005, INF-006), Sprint 9 | RID-005, RID-004 | [RID-003.md](RID-003.md) *(needs rewrite)* |
...
| RID-004 | Merge Community Signals into Composite Scoring | FEATURE | python-implement | P1 | M | 180 | RID-005 | Sprint 8 | [RID-004.md](RID-004.md) *(needs rewrite)* |
...
- **RID-004: Merge Community Signals into Composite Scoring** — Read the reconciled signals (`mentionFrequencyScore`, aggregated aspect scores, authority-weighted sentiment) directly from `curated_routes` and feed them into Sprint 8's composite scoring pipeline. Similar role to the old plan but reads from reconciled `curated_routes` fields rather than a separate NLP output table.

### Tasks Removed from the Old Plan

The following tasks from the previous Sprint 10 plan are **removed** in this rewrite. They are all now folded into Sprint 9's single `PostExtraction` LLM call:
...
- Separate GitHub Actions community ingest cron workflow task — scheduling belongs to Sprint 12 orchestrator.
...
- Sprint 9: Community Sources — Ingestion (populates `route_posts_raw` with `PostExtraction` artifacts)

**Blocks:**
- Sprint 8: Scoring & Calibration (consumes `mentionFrequencyScore` and reconciled aspect signals)
- Sprint 12: Orchestrator & E2E Integration (match + reconcile are pipeline stages)
...
- [ ] User has approved proceeding to Sprint 11

---

## Notes

- **All custom NLP is upstream in Sprint 9's `PostExtraction` call.** This epic does not classify sentiment, score aspects, detect attributes, or extract surface type — it only matches posts to routes, reconciles conflicts, and merges signals.
...
- **LLM arbitration for mid-confidence matches** shares the same threshold policy as Sprint 6 dedup (0.75–0.92). Keep the thresholds in a shared config module so they can be tuned together.
