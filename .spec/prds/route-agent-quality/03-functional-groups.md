...
| Agent Quality | **AGT** | The conversation promise: the Mastra rebuild of the agent layer on a Sonnet-class orchestrator tier (regex intent routing deleted, deterministic routing pipeline preserved as tools, in-session memory); location-grounded discovery; one-question interrogation when ambiguous; honest distances and thin-data candor; the real failed transcripts as the regression eval suite with wired traces. |
...
| Agent Quality | AGT | 5 (UC-AGT-01 … UC-AGT-05) |
| **Total** | | **25** |
