...
| 3.1.1 | 2026-07-11 | **Execution-strength re-review remediation (5-agent, +aisdk-planner).** 06: z.ai custom-provider structured-output proof + `thinkingFormat:'zai'` failure path (text-mode JSON fallback) + weather-handler forecast-window extension. 08: risk #21 extended with the structured-output-through-reasoning-wrapper failure mode; risk #1 gains rate-limit/backoff + `--max-cost`. 01: memory escape-hatch contradiction resolved (Agent instantiated `memory:undefined`; fallback is the risk-#11 bespoke loop, not an adapter). 03: geospatial re-index **migration gate** (point-count == rider-ready-count assert before SURF gate) + `agent_memory` escape-hatch removed. 04: `--max-cost` + rate-limit/backoff driver contract, `getRouteWeather` forecast note, `{dryRun?}` on all hygiene mutations. 11: §5b numeric **ceiling values** + cloud-deploy measurement method + redaction grep patterns; §5c grader-type labels (deterministic vs LLM-judge, judge = informational) + z.ai proof deliverable. 12: PROMPT_VERSION bump requires real-API smoke (not just fixtured). +3 criteria (T-AGT-024/025, T-REC-019). |
