...
**Batch-governance contract (v3.1.1 — the `--all` runs are overnight + operator-unattended):**
every batch driver accepts (a) **`--max-cost=N`** — a hard cost circuit-breaker (default
≤ ~1.5× projected) that halts with `continueCursor` preserved when projected+actual spend
crosses N; the backfill report returns `totalCostUSD`; (b) a **rate-limit / backoff contract** —
per-provider QPS with jitter, exponential backoff honoring `Retry-After`, `--max-parallel`
default 1, halt-after-N-consecutive-failures with `continueCursor` (Google Routes ~10 QPS; a 2 am
429 must back off and resume, not stop-and-wait for a human). Every **mutating hygiene/pipeline
pass accepts `{dryRun?}`** → returns a preview change-set that writes nothing and matches the
committed run (not just `dedupeGroups`). Asserted by T-REC-019 + the strengthened T-HYG rows.
...
| **getRouteWeather** | `{ polyline:{lat,lng}[], departureTimeMs:number }` | `{ status:'ok', segments[], routeWeatherSummary } \| { status:'unavailable' }` | Wraps existing tool (already errors-as-data), **extended (v3.1.1) to fetch the `daily`/`hourly` forecast window for a future `departureTimeMs`** — the handler is current-only today (`convex/actions/weather.ts:79` requests only `current=`; the Open-Meteo `/forecast` endpoint supports `daily=`/`hourly=`), so this is a real extension, not a pure wrap; ≤5 sample points; deterministic summary. Powers the volunteered go/no-go (UC-AGT-04, T-AGT-019). | `WEATHER_UNAVAILABLE` | **Wrapped** |
