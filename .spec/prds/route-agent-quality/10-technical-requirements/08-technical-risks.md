...
| 15 | **Model-reference form mismatch** — Mastra cannot consume a pi-ai `Model` object; if the `orchestrator` tier is built on pi-ai, the Agent won't instantiate | Med | High (blocks wiring) | Tier map returns Mastra ModelRouter strings for EVERY tier — pi-ai removed entirely (v3.0.2); each tier verified by one real completion (orchestrator in the §5b spike; geometry/classifier before their first batch). |
