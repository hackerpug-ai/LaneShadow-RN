...
| UC-AGT-02 | Ground discovery in the rider's location | Center always resolved (session location or geocoded place); radius + nearest |
| UC-AGT-03 | Interrogate when intent is ambiguous | One targeted clarifying question instead of guessing or deflecting |
| UC-AGT-04 | Be honest about distance and thin data | Real distances stated; no false proximity; custom-route fallback offered |
...
alternative and an offer to plan a custom route through the routing pipeline instead.
