...
**Human testing policy:** Human tests for non-sandbox code must include real-device E2E steps. Simulator/emulator checks may remain as supporting evidence, but any gate that exercises live app flows, auth, Convex, Mapbox, persistence, location, or external services must include a real-device E2E path with recorded evidence. For iOS, use the project WDA pattern documented in `docs/REAL_DEVICE_E2E.md`; for Android, use physical-device evidence or mark the step MANUAL/BLOCKED until an equivalent Android device harness exists. Sandbox-only visual/component gates may continue to use simulator/emulator snapshot tooling.
...
| AUTH-S03-T11 | Real-device WDA E2E gate — codify every Sprint 03 human test step as a WDA-backed result artifact with PASS/FAIL/BLOCKED/MANUAL status, screenshots/source dumps, and diagnostics for non-sandbox auth + Convex flows | swift-implementer | 240 min |
