# RR-S09-DR-T01 — Reconcile route-results-screen variant naming + verify reference assets
...
> **PRD Refs:** UC-FID-01 (route-results-screen variants), Sprint 09 — Map View Route Results State
...
Downstream capture tests (RR-S09-IOS-T05, RR-S09-AND-T05) MUST align to a single canonical set. This task locks the canonical set to the design-system README + existing PNG filenames (those have already been generated, are token-pure, and match the HTML mockup), updates the ROADMAP gate text accordingly (or annotates the gate to reference canonical names), confirms `pnpm design:references --screens route-results-screen` is idempotent against the current HTML, and produces the authoritative variant-matrix table that capture tests and view-model fixtures must consume.

The task is **non-implementation infrastructure**: it does not write Swift or Kotlin code, does not run xcodebuild or gradle, and does not modify the design HTML itself unless reconciliation reveals a missing variant. The single point of authority output is the canonical variant table embedded in this sprint's `SPRINT.md` (Per-Task Design Files row), which Sprint 09 implementer tasks consult.
...
- MUST publish the authoritative seven-variant table to `tasks/sprint-09-route-results-screen/VARIANTS.md` mapping ROADMAP gate label → canonical filename → state-machine description → theme → light/dark applicability
...
- MUST flag any variant that exists in the canonical filename set but NOT in `route-results-screen.html` (or vice versa) as a BLOCKER and surface it in `gate-evidence/RR-S09-DR-T01.md` for sprint discussion
...
- NEVER overwrite `.spec/design/system/views/route-results-screen/README.md` — the design-system README is the source of truth for naming
...
**Objective:** Lock the canonical seven-variant matrix for route-results-screen by reconciling the ROADMAP gate text with the design-system README + existing PNG reference set, verify `pnpm design:references --screens route-results-screen` is idempotent against the current HTML, and publish the authoritative table to `VARIANTS.md` for downstream capture tests + view-model fixtures.

**Success State:** `.spec/prds/v3-integration/tasks/sprint-09-route-results-screen/VARIANTS.md` exists and contains a complete seven-row table mapping {ROADMAP label, canonical PNG filename, state machine description, theme, light/dark applicability}. `pnpm design:references --screens route-results-screen` exits 0 with zero PNG diffs (idempotent). The seven canonical PNG filenames + matching `.annotations.json` files are present in `.spec/design/system/refs/route-results-screen/`. `gate-evidence/RR-S09-DR-T01.md` records the verification run output and lists any reconciliation findings.

## Acceptance Criteria

### AC-1 — Canonical seven-variant table published to VARIANTS.md

**GIVEN** Sprint 09 begins task expansion
**WHEN** `RR-S09-DR-T01` executes
**THEN** `.spec/prds/v3-integration/tasks/sprint-09-route-results-screen/VARIANTS.md` exists with a markdown table containing exactly seven rows (one per canonical variant) and columns: `ROADMAP label`, `Canonical filename stem`, `State machine description`, `Theme`, `Reference asset path`
**Verify:** `test -f /Users/justinrich/Projects/LaneShadow/.spec/prds/v3-integration/tasks/sprint-09-route-results-screen/VARIANTS.md && grep -c '^| ' /Users/justinrich/Projects/LaneShadow/.spec/prds/v3-integration/tasks/sprint-09-route-results-screen/VARIANTS.md` returns ≥ 8 (1 header row + 7 data rows + alignment row)
...
**WHEN** `route-results-screen.html` is parsed
...
**Verify:** `grep -c 'data-variant=' /Users/justinrich/Projects/LaneShadow/.spec/design/system/views/route-results-screen/route-results-screen.html` returns ≥ 7 OR `gate-evidence/RR-S09-DR-T01.md` enumerates the missing attributes as a blocker

### AC-5 — Reconciliation findings recorded as gate evidence

**GIVEN** the reconciliation between ROADMAP gate text and canonical filenames
**WHEN** `RR-S09-DR-T01` completes
**THEN** `.spec/prds/v3-integration/tasks/sprint-09-route-results-screen/gate-evidence/RR-S09-DR-T01.md` exists with: (a) command output from `pnpm design:references --screens route-results-screen`, (b) list of any reconciliation findings (terminology drift, missing variants, missing data attributes), (c) explicit statement either "no blockers" or enumerated blockers with proposed resolution
**Verify:** `test -f /Users/justinrich/Projects/LaneShadow/.spec/prds/v3-integration/tasks/sprint-09-route-results-screen/gate-evidence/RR-S09-DR-T01.md && grep -l "design:references" /Users/justinrich/Projects/LaneShadow/.spec/prds/v3-integration/tasks/sprint-09-route-results-screen/gate-evidence/RR-S09-DR-T01.md`

## Test Criteria

| ID | Statement | Maps to AC | Verify | Type |
|---|---|---|---|---|
| TC-1 | VARIANTS.md exists with 7-row markdown table containing the required 5 columns | AC-1 | `test -f /Users/justinrich/Projects/LaneShadow/.spec/prds/v3-integration/tasks/sprint-09-route-results-screen/VARIANTS.md && grep -c '^\|' /Users/justinrich/Projects/LaneShadow/.spec/prds/v3-integration/tasks/sprint-09-route-results-screen/VARIANTS.md` ≥ 8 | happy_path |
...
| TC-7 | gate-evidence/RR-S09-DR-T01.md exists and references the design:references command output | AC-5 | `test -f /Users/justinrich/Projects/LaneShadow/.spec/prds/v3-integration/tasks/sprint-09-route-results-screen/gate-evidence/RR-S09-DR-T01.md && grep -q 'design:references' /Users/justinrich/Projects/LaneShadow/.spec/prds/v3-integration/tasks/sprint-09-route-results-screen/gate-evidence/RR-S09-DR-T01.md` | happy_path |
...
| `RULES.md` | "Design Review Pipeline — View Snapshot Testing" | Planner contract for design-review pipeline expansion per sprint |
| `.spec/prds/v3-integration/tasks/sprint-08-planning-state/PLAN-S08-DR-T01-regenerate-planning-screen-references.md` | all | Sprint 08 prior-art for the same kind of reconciliation task |
...
- Sprint 08 prior-art: PLAN-S08-DR-T01

**Interaction Notes:** REQUIRED READING: `.spec/design/system/views/route-results-screen/route-results-screen.html` + `.spec/design/system/views/route-results-screen/README.md`. No new user-facing interactions are introduced — this task verifies the existing reference set and locks the canonical variant matrix that downstream capture tests will hit.

**Pattern:** Sprint 08 PLAN-S08-DR-T01 reconciliation pattern. The Sprint 08 task ran `pnpm design:references` to verify idempotency, then produced the canonical variant matrix in the sprint folder's SPRINT.md. Mirror that approach here, but with `VARIANTS.md` as a dedicated artifact (the matrix is referenced from multiple downstream tasks).
...
| AC-5 | `test -f gate-evidence/RR-S09-DR-T01.md && grep -q 'design:references' gate-evidence/RR-S09-DR-T01.md` |

## Agent Assignment

**Agent:** frontend-designer
**Rationale:** This is design-system reconciliation work — verifying canonical names, idempotency of reference generation, and authoring a variant matrix. No native code, no Convex code, no E2E test code. Matches `frontend-designer`'s mandate per `RULES.md` ("Standalone visual exploration; do not assign sprint implementation, verification, token pipelines, registries, or stateful UI work to this agent.") — this task is exploratory/verification design work, NOT sprint implementation. Output is markdown + verified PNG idempotency, both of which fall inside the agent's safe scope. The reviewer for this task should be a project maintainer who reads `VARIANTS.md` for correctness; no auto-reviewer is dispatched because there's no code to lint.

## Coding Standards

- `RULES.md` §"Design Review Pipeline — View Snapshot Testing" (planner contract)
- `RULES.md` §"Cross-Platform Component Parity" (canonical sandbox story ID naming — mirrors capture filename naming)
- `.spec/design/system/views/route-results-screen/README.md` (canonical variant matrix)
...
**Blocks:**
- RR-S09-IOS-T05 (iOS capture tests consume the canonical variant matrix from VARIANTS.md)
- RR-S09-AND-T05 (Android capture tests consume the same matrix)
...
{
  "requirements": [
    {"id":"AC-1","type":"acceptance_criterion","description":"VARIANTS.md exists with 7-row markdown table mapping ROADMAP label to canonical filename to state machine description to theme to reference asset path","verify":"test -f .spec/prds/v3-integration/tasks/sprint-09-route-results-screen/VARIANTS.md && grep -c '^|' .spec/prds/v3-integration/tasks/sprint-09-route-results-screen/VARIANTS.md returns >= 8","satisfied":null,"evidence":null,"remediation":null,"last_evaluated_cycle":null,"last_evaluated_commit":null,"maps_to_ac":null},
    ...
    {"id":"AC-5","type":"acceptance_criterion","description":"gate-evidence/RR-S09-DR-T01.md records the design:references command output, any reconciliation findings, and an explicit no-blockers or blockers-enumerated statement","verify":"test -f gate-evidence/RR-S09-DR-T01.md && grep -q 'design:references' gate-evidence/RR-S09-DR-T01.md","satisfied":null,"evidence":null,"remediation":null,"last_evaluated_cycle":null,"last_evaluated_commit":null,"maps_to_ac":null},
    {"id":"TC-1","type":"test_criterion","description":"VARIANTS.md has 7 data rows + 1 header row + 1 alignment row in markdown table","verify":"grep -c '^|' .spec/prds/v3-integration/tasks/sprint-09-route-results-screen/VARIANTS.md >= 8","satisfied":null,"evidence":null,"remediation":null,"last_evaluated_cycle":null,"last_evaluated_commit":null,"maps_to_ac":"AC-1"},
    ...
    {"id":"TC-7","type":"test_criterion","description":"gate-evidence/RR-S09-DR-T01.md exists and references design:references command output","verify":"test -f gate-evidence/RR-S09-DR-T01.md && grep -q 'design:references' gate-evidence/RR-S09-DR-T01.md","satisfied":null,"evidence":null,"remediation":null,"last_evaluated_cycle":null,"last_evaluated_commit":null,"maps_to_ac":"AC-5"}
  ]
}
-->
