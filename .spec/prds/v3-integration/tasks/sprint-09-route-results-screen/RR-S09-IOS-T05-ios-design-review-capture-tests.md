# RR-S09-IOS-T05 — iOS DesignReviewCaptureTests for route-results-screen variants
...
> **PRD Refs:** UC-FID-01 (route-results-screen variants), Sprint 09 — Design Review Gate

## Background

Add XCUITest capture methods to `ios/LaneShadowUITests/DesignReview/DesignReviewCaptureTests.swift` that drive `RouteResultsScreen` to each of the 7 canonical variants (from RR-S09-DR-T01's `VARIANTS.md`) and capture a PNG per `(variant, theme)` tuple. Each capture is fed into the `pnpm design:review` pipeline for vision-LLM evaluation against the reference assets in `.spec/design/system/refs/route-results-screen/`. The capture pipeline must produce 7 captures matching the canonical filenames.

## Critical Constraints

**MUST:**
- MUST add 7 new test methods to `ios/LaneShadowUITests/DesignReview/DesignReviewCaptureTests.swift`: `test_routeResultsScreen_default_best_pre_selected_light`, `test_routeResultsScreen_alt1_tapped_sage_promoted_light`, `test_routeResultsScreen_default_dark`, `test_routeResultsScreen_refining_light`, `test_routeResultsScreen_two_candidates_light`, `test_routeResultsScreen_weather_divergent_light`, `test_routeResultsScreen_message_dismissed_light`
- MUST drive each test by opening the canonical sandbox story `templates.route-results-screen.{variant}-{theme}` registered in RR-S09-IOS-T02
...
- MUST write captures to `ios/build/design-review/captures/route-results-screen/` (or the existing capture output directory used by Sprint 05+)
...
- NEVER skip the alt1-tapped variant's interaction step (the test MUST drive the alt-card tap to trigger the S02 promotion before capturing)
...
- STRICTLY follow `RULES.md` §"Design Review Pipeline — View Snapshot Testing" — every variant must have a `data-variant` or accessibility identifier that the test can target
...
**Objective:** Add 7 XCUITest capture methods to `DesignReviewCaptureTests.swift` covering every canonical route-results-screen variant; each test drives the sandbox story to the appropriate state, captures a PNG, and writes it to the design-review capture output directory matching the canonical filenames.

**Success State:** `xcodebuild test -only-testing:LaneShadowUITests/DesignReview/DesignReviewCaptureTests -only-test-class-method test_routeResultsScreen_*` produces 7 PNGs under `ios/build/design-review/captures/route-results-screen/`; `pnpm design:review --screens route-results-screen` runs end-to-end against these captures and produces a `report.json` with one entry per variant; zero `high`-severity issues across all 7 variants (gate-blocker per Sprint 09 Design Review Gate).

## Acceptance Criteria

### AC-1 — Seven test methods exist with canonical names

**GIVEN** RR-S09-DR-T01 has published the canonical 7-variant matrix
**WHEN** `ios/LaneShadowUITests/DesignReview/DesignReviewCaptureTests.swift` is parsed
**THEN** the file contains all 7 test method declarations with canonical names
**Verify:** `grep -c '^[[:space:]]*func test_routeResultsScreen' ios/LaneShadowUITests/DesignReview/DesignReviewCaptureTests.swift` returns 7
...
**GIVEN** the `test_routeResultsScreen_alt1_tapped_sage_promoted_light` test method
**WHEN** the test executes
**THEN** the test taps the alt1 card BEFORE capturing the PNG; the captured PNG shows the sage polyline as solid-bold and the alt1 card with sage stripe + compass chip
...
**GIVEN** the `test_routeResultsScreen_refining_light` test method
**WHEN** the test executes
**THEN** the test focuses the chat input AND ensures the refine primer chips are visible BEFORE capturing
...
**GIVEN** the `test_routeResultsScreen_message_dismissed_light` test method
**WHEN** the test executes
**THEN** the test taps the dismiss chevron on `LSNavigatorMessage` BEFORE capturing; the captured PNG shows the Recall chip bottom-anchored in copper
...
|---|---|---|---|---|
| TC-1 | Seven test methods with canonical names declared in DesignReviewCaptureTests.swift | AC-1 | `grep -c '^[[:space:]]*func test_routeResultsScreen' ios/LaneShadowUITests/DesignReview/DesignReviewCaptureTests.swift` returns 7 | happy_path |
...
| `.spec/prds/v3-integration/tasks/sprint-09-route-results-screen/VARIANTS.md` | all | Canonical 7-variant matrix from RR-S09-DR-T01 |
...
| `.spec/prds/v3-integration/tasks/sprint-08-planning-state/PLAN-S08-IOS-T05-ios-design-review-capture-tests.md` | all | Sprint 08 sibling — same architecture, different content |
...
**References:**
- `.spec/prds/v3-integration/tasks/sprint-09-route-results-screen/VARIANTS.md` (canonical 7-variant matrix)
...
**Interaction Notes:** Each variant requires specific XCUITest interactions to reach the captured state: S01 just opens the sandbox story; S02 requires tapping the alt1 card; S03 toggles dark mode before capture; S04 focuses the chat input; V01 uses a 2-option fixture; V02 uses a 3-option fixture with mixed weather; V03 taps the dismiss chevron. All interactions happen via `XCUIElement.tap()` against accessibility identifiers established in RR-S09-IOS-T02 and RR-S09-IOS-T04.

**Pattern:** Sprint 08 PLAN-S08-IOS-T05 — same test architecture, swap story IDs and interactions.
...
**Anti-Pattern:** Hand-authoring PNGs; capturing pre-interaction state for interaction-required variants (S02, S04, V03); writing captures outside the canonical output directory; skipping the manifest registration.

## Verification Gates

| AC | Command |
|---|---|
| AC-1 | `grep -c '^[[:space:]]*func test_routeResultsScreen' ios/LaneShadowUITests/DesignReview/DesignReviewCaptureTests.swift` returns 7 |
| AC-2 | `xcodebuild test -only-testing:LaneShadowUITests/DesignReview/DesignReviewCaptureTests` (filtered to route-results) + `ls ios/build/design-review/captures/route-results-screen/ \| grep -c '\.png$'` returns 7 |
...
- RR-S09-IOS-T02 (sandbox stories registered with canonical IDs + accessibility identifiers)
...
{
  "requirements": [
    {"id":"AC-1","type":"acceptance_criterion","description":"Seven test methods exist with canonical names matching VARIANTS.md","verify":"grep -c '^[[:space:]]*func test_routeResultsScreen' ios/LaneShadowUITests/DesignReview/DesignReviewCaptureTests.swift returns 7","satisfied":null,"evidence":null,"remediation":null,"last_evaluated_cycle":null,"last_evaluated_commit":null,"maps_to_ac":null},
    ...
    {"id":"AC-3","type":"acceptance_criterion","description":"Alt-tapped variant capture reflects post-selection state (sage promotion)","verify":"pnpm design:review --screens route-results-screen + jq filter alt1-tapped severity != high","satisfied":null,"evidence":null,"remediation":null,"last_evaluated_cycle":null,"last_evaluated_commit":null,"maps_to_ac":null},
    ...
    {"id":"AC-5","type":"acceptance_criterion","description":"Message-dismissed variant capture shows Recall chip bottom-anchored","verify":"pnpm design:review --screens route-results-screen + jq filter message-dismissed severity != high","satisfied":null,"evidence":null,"remediation":null,"last_evaluated_cycle":null,"last_evaluated_commit":null,"maps_to_ac":null},
