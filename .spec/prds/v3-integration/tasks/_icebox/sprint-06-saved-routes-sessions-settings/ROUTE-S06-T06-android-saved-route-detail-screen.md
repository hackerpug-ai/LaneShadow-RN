...
- android/app/src/main/java/com/laneshadow/data/dto/SavedRouteDetailDto.kt (NEW — DTO mirroring `db.savedRoutes.getSavedRouteDetail` return shape per `convex/db/savedRoutes.ts:385-396` validator)
...
⚠️ Ask First:
- If `db.savedRoutes.getSavedRouteDetail` return shape diverges from `savedRouteDetailViewValidator` in `convex/db/savedRoutes.ts:385`
