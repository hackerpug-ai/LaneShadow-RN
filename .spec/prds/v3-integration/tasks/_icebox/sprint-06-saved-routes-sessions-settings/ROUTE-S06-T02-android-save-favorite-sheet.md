...
⚠️ Ask First:
- If the saveRoute mutation Convex schema diverges from `{planInput, routeSnapshot, routeIndex, snapshotMeta}` validators in `convex/db/savedRoutes.ts:422`
...
- saveRoute payload validates against `convex/db/savedRoutes.ts:422` mutation args
