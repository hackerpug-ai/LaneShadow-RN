# IDLE-S06-REM-CVX-T02 — Mapbox Search Box idle autocomplete actions
...
- **NEVER** add Google Places SDK/API dependencies or call Google Places endpoints for this Sprint 06 remediation.
...
- [ ] AC-1: `actions/places:suggestPlaces` returns at most three mapped suggestions for query length >= 2 (PRIMARY)
- [ ] AC-2: `suggestPlaces` returns `[]` without upstream fetch for trimmed queries shorter than 2 characters
- [ ] AC-3: `suggestPlaces` forwards `session_token`, `country=US`, `language=en`, `limit=3`, and optional `proximity=lng,lat`
- [ ] AC-4: `actions/places:retrievePlace` maps a Mapbox suggestion id to `{id,name,label,lat,lng,featureType}`
- [ ] AC-5: Mapbox upstream failures return typed Convex errors without leaking `MAPBOX_ACCESS_TOKEN`
...
- **Blocks:** IDLE-S06-REM-IOS-T02, IDLE-S06-REM-AND-T02, IDLE-S06-REM-GATE-T02
