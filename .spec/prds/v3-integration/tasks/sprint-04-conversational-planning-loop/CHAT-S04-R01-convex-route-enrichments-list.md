...
- MUST define a public `query` named `list` in `convex/db/routeEnrichments.ts` accepting `{ routePlanId: v.id('route_plans') }`
...
AC-1: Public list query exists with correct shape contract [PRIMARY]
  ...
  TDD_STATE:     none
  TEST_FILE:     convex/db/routeEnrichments.test.ts
...
AC-5: Index-backed lookup (no table scan)
  ...
  TDD_STATE:     none
  TEST_FILE:     convex/db/routeEnrichments.ts
...
writeAllowed:
- convex/db/routeEnrichments.ts (MODIFY — add `list` query export + ownership helper)
- convex/db/routeEnrichments.test.ts (CREATE OR MODIFY — add tests for AC-1..AC-5)

writeProhibited:
- convex/_generated/** — auto-generated; will be regenerated by convex:dev
- convex/schema.ts — no schema changes required; existing `by_routePlanId` index suffices
- convex/guards.ts — reserved for CHAT-S04-R03
- ios/** + android/** — mobile callers out of scope
- Any file outside convex/db/routeEnrichments.* and the explicit list above
...
- convex/db/routeEnrichments.ts (MODIFY): add public `list` query handler
- convex/db/routeEnrichments.test.ts (NEW): convex-test coverage for AC-1..AC-5
...
Gate 6: Biome lint clean
  Command: pnpm exec biome check --no-errors-on-unmatched convex/db/routeEnrichments.ts convex/db/routeEnrichments.test.ts
  Expected: Exit 0.
...
<!--
{
  ...
  "requirements": [
    ...
    {"id": "AC-5", "type": "acceptance_criterion", "description": "GIVEN route_enrichments has by_routePlanId index WHEN list handler is read THEN it uses withIndex (not filter)", "verify": "pnpm exec biome check --no-errors-on-unmatched convex/db/routeEnrichments.ts", "satisfied": false, "evidence": null, "remediation": null},
    ...
    {"id": "TC-5", "type": "test_criterion", "description": "Static check: list uses withIndex('by_routePlanId') and returns validator is explicit v.object", "maps_to_ac": "AC-5", "verify": "pnpm exec biome check --no-errors-on-unmatched convex/db/routeEnrichments.ts", "satisfied": false, "evidence": null, "remediation": null}
  ]
}
-->
================================================================================
