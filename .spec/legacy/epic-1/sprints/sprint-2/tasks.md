...
**Sprint**: `.spec/epics/epic-1/sprints/sprint-2/spec.md`
**Source of truth**: `.spec/epics/epic-1/trd/phase-1-core.md` + `.spec/epics/epic-1/EPIC-ROADMAP.md`
**Sprint 1 handoff (what already exists)**: `.spec/epics/epic-1/sprints/sprint-1/handoff.md`
...
#### Acceptance Criteria
- [x] `api.db.routesPlan.getPlanInit` exists after Convex codegen.
- [x] Return shape matches `PlanInitView` from `types/routes.ts`.
- [x] Unauthenticated callers fail deterministically (no silent nulls).

#### Files to Create / Modify
- **Create**: `convex/db/routesPlan.ts`

#### Testing Requirements
- [x] Minimal: `pnpm tsc -p convex/tsconfig.json --noEmit` passes.
...
#### Acceptance Criteria
- [x] `api.db.savedRoutes.getSavedRoutesList` exists after Convex codegen.
- [x] Uses the `by_ownerType_and_ownerId` index via the existing internal helper (no table scan filters).
- [x] List is bounded (choose a hard cap, e.g. 50) and summary-only.
- [x] Returns `SavedRoutesListView` shape (matches `types/routes.ts`).
...
#### Acceptance Criteria
- [x] `api.db.savedRoutes.getSavedRouteDetail` exists after Convex codegen.
- [x] Missing and unauthorized both return `null` (no existence leak).
- [x] Return shape matches `SavedRouteDetailView` from `types/routes.ts`.
...
#### Acceptance Criteria
- [x] `api.db.savedRoutes.saveRoute` exists after Convex codegen.
- [x] Inserts always write POC owner fields consistently (ownerType=user, visibility=private, ownerId=viewer).
...
#### Acceptance Criteria
- [x] `api.db.savedRoutes.renameRoute` and `api.db.savedRoutes.deleteRoute` exist after Convex codegen.
- [x] Missing/not-owned cannot be distinguished from missing (NOT_FOUND semantics).
- [x] Only metadata changes on rename (no snapshot mutation).
