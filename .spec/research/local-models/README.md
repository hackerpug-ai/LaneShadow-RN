...
## ⚠️ Methodology Note — Environment Bias (added 2026-04-10)

**All latency findings in this directory were measured on a 2026 MacBook Pro (Apple Silicon, MLX runtime). They are not valid for mobile.**

The production target is a React Native app on iPhone and Android. MLX does not run on iOS or Android. The real mobile runtimes are Core ML (iOS) and ONNX/TFLite (Android), running on hardware with ~4–6× lower memory bandwidth than the Mac used for testing.

**Corrected estimates:**
- Observed Qwen latency on Mac: ~1.5–2.5s
- Estimated on iPhone 15/16 Pro: ~8–15s (not measured, not validated)
- Haiku API latency: ~1.1s on Mac / ~1.5–3s on LTE — network-bound, transfers reliably

**Accuracy findings are largely portable. Latency findings are not. All latency claims in results docs should be read as "Mac MLX only" until device benchmarks are run.**

Full finding: [`ENVIRONMENT_BIAS_FINDING_2026-04-10.md`](ENVIRONMENT_BIAS_FINDING_2026-04-10.md)
