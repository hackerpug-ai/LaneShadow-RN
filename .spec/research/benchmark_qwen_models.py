#!/usr/bin/env python3
"""
LLM Model Benchmark for Motorcycle Route Suggestion - Qwen3.5 Edition
Tests Qwen3.5 models against previous baseline models (Llama 3.2, Phi-3)
Measures structured JSON output quality for motorcycle route planning
"""
...
# Models to benchmark - 2026 editions
LOCAL_MODELS = [
    # 2026 models (new) - MLX-community versions
    "mlx-community/Qwen3.5-0.8B-OptiQ-4bit",
    "mlx-community/Qwen3.5-4B-MLX-4bit",

    # 2025 models (baseline)
...
def get_model_size(model_name: str) -> Dict[str, float]:
    ...
    # Try MLX cache first
    mlx_cache = Path.home() / ".cache" / "mlx" / "models"
    model_path = None

    # Check if it's in MLX cache
    for candidate in mlx_cache.rglob("*"):
        if model_name.replace("/", "--") in str(candidate):
...
def save_results(all_results: List[Dict[str, Any]], output_path: str):
    ...
    with open(output_file, 'w') as f:
        json.dump({
            ...
            "benchmark_type": "motorcycle_route_structured_json_2026",
            "benchmark_description": "Qwen3.5 vs 2025 models for motorcycle route planning",
            "system_info": {
...
def main():
    ...
    print("="*60)
    print("LLM Motorcycle Route Benchmark - 2026 Edition")
    print("Testing Qwen3.5 vs 2025 Baselines")
    print("="*60)
    print(f"Local models: {len(LOCAL_MODELS)}")
    print(f"Prompts per model: {len(ROUTE_PROMPTS)}")
    print(f"Output: .spec/research/llm_benchmark_results_2026.json")
    print("="*60)
    ...
    # Save results
    save_results(all_results, ".spec/research/llm_benchmark_results_2026.json")
