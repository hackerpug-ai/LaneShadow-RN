...
export function validateEnums(params: IntentParams): IntentParams {
  ...
  if (params.archetypes && params.archetypes.length > 0) {
    validated.archetypes = params.archetypes.filter((a) => VALID_ARCHETYPES.includes(a as any))
  }

  // Filter states to valid values only
  if (params.states && params.states.length > 0) {
    validated.states = params.states.filter((s) => VALID_STATES.includes(s as any))
  }
