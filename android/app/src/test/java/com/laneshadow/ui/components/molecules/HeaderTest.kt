...
/**
 * TDD tests for Header component
 *
...
class HeaderTest {
    ...
    /**
     * AC-1: Component renders with title, menu button, and layout structure
     * GIVEN: App is running and component is mounted
     * WHEN: Header is rendered with title and menu callback
     * THEN: Component displays title, menu button, and proper layout structure
     */
    @Test
    fun testHeaderRendersWithTitleAndMenuButton() {
        ...
        // Verify menu button is present with test ID
        composeTestRule.onNodeWithTag("test-header-menu-button").assertIsDisplayed()

        // Verify right spacer is present
        composeTestRule.onNodeWithTag("test-header-right-spacer").assertIsDisplayed()

        // Verify menu button press works
        composeTestRule.onNodeWithTag("test-header-menu-button").performClick()
        assert(menuPressed) { "Menu press callback should be invoked" }
    }

    /**
     * AC-2: Component uses correct theme tokens for colors, spacing, radius, and typography
     ...
     */
    @Test
    fun testHeaderUsesThemeTokens() {
        composeTestRule.setContent {
            CompositionLocalProvider(LocalLaneShadowTheme provides testTheme) {
                Header(
                    title = "Theme Test",
                    onMenuPress = {},
                    testId = "theme-test-header",
                )
            }
        }
        ...
        composeTestRule.onNodeWithTag("theme-test-header-menu-button").assertIsDisplayed()
    }

    /**
     * AC-3: Component has proper accessibility with content description
     * GIVEN: Screen reader is active
     * WHEN: Component is rendered
     * THEN: Menu button has proper content description
     */
    @Test
    fun testHeaderAccessibility() {
        composeTestRule.setContent {
            CompositionLocalProvider(LocalLaneShadowTheme provides testTheme) {
                Header(
        ...
        // Verify menu button has content description
        composeTestRule.onNodeWithContentDescription("Menu").assertIsDisplayed()
    }

    /**
     * AC-4: Component applies correct styling (background, border, height, padding)
     * GIVEN: Design spec defines header appearance
     * WHEN: Component is rendered
     * THEN: Header has background, bottom border, correct height, and padding
     */
    @Test
    fun testHeaderStyling() {
        composeTestRule.setContent {
            CompositionLocalProvider(LocalLaneShadowTheme provides testTheme) {
                Header(
                    title = "Styled Header",
        ...
        // Verify header container is rendered
        composeTestRule.onNodeWithTag("styled-header").assertIsDisplayed()
        composeTestRule.onNodeWithText("Styled Header").assertIsDisplayed()
    }

    /**
     * AC-5: Menu button has correct dimensions and pressed state styling
     * GIVEN: Menu button spec defines 44x44 size and pressed state
     * WHEN: Menu button is rendered
     * THEN: Button has correct size and pressed background
     */
    @Test
    fun testMenuButtonDimensionsAndPressedState() {
        composeTestRule.setContent {
            CompositionLocalProvider(LocalLaneShadowTheme provides testTheme) {
                Header(
                    title = "Menu Button Test",
                    onMenuPress = {},
                    testId = "menu-btn-test",
                )
            }
        }

        // Verify menu button exists and is clickable
        composeTestRule.onNodeWithTag("menu-btn-test-menu-button").assertIsDisplayed()
    }
}
