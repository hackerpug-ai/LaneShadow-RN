...
/**
 * TDD tests for InfoToast component
 *
...
class InfoToastTest {
    ...
    /**
     * AC-1: InfoToast renders with title, description, and close button
     *
     * GIVEN: InfoToast with title, description, and showCloseButton=true
     * WHEN: Composed
     * THEN: Displays title, description, close icon, and info icon
     */
    @Test
    fun test_rendersWithTitleDescriptionAndCloseButton() {
        ...
        composeTestRule.onNodeWithText("Info Title")
            .assertIsDisplayed()

        // Verify description is displayed
        composeTestRule.onNodeWithText("Info description text")
            .assertIsDisplayed()

        // Verify close button exists and is clickable
        composeTestRule.onNodeWithContentDescription("Close info toast")
            .assertExists()
            .performClick()

        assert(closeClicked) { "onClose callback should be invoked" }
    }

    /**
     * AC-2: InfoToast hides close button when showCloseButton=false
     *
     * GIVEN: InfoToast with showCloseButton=false
     * WHEN: Composed
     * THEN: Does not display close button
     */
    @Test
    fun test_hidesCloseButtonWhenDisabled() {
        composeTestRule.setContent {
            CompositionLocalProvider(LocalLaneShadowTheme provides testTheme) {
                InfoToast(
                    title = "Info Title",
    ...
    /**
     * AC-3: InfoToast has proper accessibility description
     *
     * GIVEN: InfoToast with title and description
     * WHEN: Composed
     * THEN: Has semantic content description combining title and description
     */
    @Test
    fun test_hasAccessibilityDescription() {
        composeTestRule.setContent {
            CompositionLocalProvider(LocalLaneShadowTheme provides testTheme) {
                InfoToast(
                    title = "Network Info",
        ...
        // Verify info icon has content description
        composeTestRule.onNodeWithContentDescription("Info icon")
            .assertExists()
    }

    /**
     * AC-4: InfoToast uses theme tokens for styling
     *
     * GIVEN: InfoToast composed with test theme
     * WHEN: Rendered
     * THEN: Uses info color for background, onPrimary for text, lg radius, level4 elevation
     */
    @Test
    fun test_usesThemeTokens() {
        composeTestRule.setContent {
            CompositionLocalProvider(LocalLaneShadowTheme provides testTheme) {
                InfoToast(
                    title = "Styled Info",
                    description = "This should use theme tokens",
                )
            }
        }

        // Verify component renders (visual tokens verified through inspection)
        composeTestRule.onNodeWithText("Styled Info")
            .assertIsDisplayed()

        composeTestRule.onNodeWithText("This should use theme tokens")
            .assertIsDisplayed()
    }
}
