...
class MinimalOverlayWidgetTest {
    ...
    @Test
    fun test_rendersCenterButton_whenNoSelection() {
        var currentValue: OverlayType? = null
        ...
        // Should render the center button
        composeTestRule.onNodeWithTag("overlay-widget").assertIsDisplayed()
        composeTestRule.onNodeWithContentDescription("Weather overlays").assertIsDisplayed()
    }

    @Test
    fun test_rendersCenterButton_whenWindSelected() {
        var currentValue: OverlayType? = OverlayType.WIND
        ...
        // Should render the center button with wind icon
        composeTestRule.onNodeWithTag("overlay-widget").assertIsDisplayed()
        composeTestRule.onNodeWithContentDescription("Weather overlays").assertIsDisplayed()
    }

    @Test
    fun test_centerButtonTogglesExpansion() {
        var currentValue: OverlayType? = null
        var expandedStates = mutableListOf<Boolean>()
        ...
        // Click center button
        composeTestRule.onNodeWithContentDescription("Weather overlays")
            .performClick()

        // Should trigger some interaction
        // (In a real scenario, we'd check for expanded state, but for now we verify the click doesn't crash)
    }

    @Test
    fun test_unavailableIconsHaveReducedOpacity() {
        var currentValue: OverlayType? = null
        ...
        // Should render without errors even with unavailable overlays
        composeTestRule.onNodeWithTag("overlay-widget").assertIsDisplayed()
    }

    @Test
    fun test_allOverlayTypesSupported() {
        val overlayTypes = listOf(
            OverlayType.WIND,
            OverlayType.RAIN,
        ...
        overlayTypes.forEach { overlayType ->
            var currentValue: OverlayType? = overlayType
            ...
            // Should render for each overlay type
            composeTestRule.onNodeWithTag("overlay-widget").assertIsDisplayed()
        }
    }
}
