...
- `LSToast` regressions are easier to catch when the dismiss recipe resolves both its visible window and exit animation from the same token-backed `motion.duration.fast` value and the test suite also rejects a reintroduced `5000` literal.
