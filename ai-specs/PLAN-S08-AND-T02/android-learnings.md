...
3. `MapAppContent` kept `controlsMode` in remembered local state across planning sessions. After toggling into chat mode, leaving planning, and re-entering with a new session id, the workbar reopened in chat mode until the session lifecycle explicitly reset it.
...
- Planning workbar mode now resets to `MapControlsMode.Map` whenever the live host leaves planning or enters a new planning session key, so re-entry always restores recenter and zoom affordances.
...
- If the planning/chat workbar mode is stored locally, key its reset to planning session lifecycle rather than to simple recomposition, or chat mode will leak across exit/re-entry.
...
- `android/app/src/main/java/com/laneshadow/ui/templates/PlanningScreen.kt` — changed `onResetMapState` to optional and added a planning session reset key so the direct template can realign its workbar mode on session changes.
- `android/app/src/main/java/com/laneshadow/ui/planning/PlanningScreenContainer.kt` — threads the real `uiState.sessionId` into `PlanningScreen` for session-aware controls reset.
...
- `android/app/src/test/java/com/laneshadow/ui/templates/PlanningScreenTest.kt` — now covers planning -> chat -> idle -> planning re-entry and asserts the workbar returns to map mode with recenter and zoom visible.
