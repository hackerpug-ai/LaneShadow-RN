You are the implementer for kb-run execution unit `UC-ATM-12-android` in an isolated worktree. This is a bounded remediation run.

Rules:
- Do not edit `.kb-run/state.json` or any `.kb-run` scheduler files.
- Work only inside the allowed task scope.
- Use the existing Android LSMap stub implementation as the starting point; do not restart from scratch.
- Read only the task file plus these source files unless a concrete compile failure forces one additional read:
...
- Do not do repo-wide searches or architecture-document hunts. You already have enough context.
- Replace the shallow LSMap tests with targeted RED tests for the reopened gaps, then implement the minimal production changes to pass them.
- Write evidence artifacts to these absolute paths:
  - `/Users/justinrich/Projects/LaneShadow/.kb-run/tasks/UC-ATM-12-android/iterations/001/evidence.md`
  - `/Users/justinrich/Projects/LaneShadow/.kb-run/tasks/UC-ATM-12-android/iterations/001/evidence-manifest.json`
...
- Worktree: `/Users/justinrich/Projects/LaneShadow/.kb-run/worktrees/UC-ATM-12-android`
- Android Mapbox dependency is already enabled and `./gradlew :app:compileDebugKotlin` currently succeeds in this repo.

Focus strictly on these reopened gaps:
- Use generated token style URLs instead of placeholder style loading.
- Implement runtime AndroidView + MapView behavior without leaking Mapbox SDK types into the public composable signature.
- Preserve MapView instance across theme changes and reload style in-place.
- Implement multi-polyline rendering with route token colors and stroke width token.
- Implement annotation rendering with status token colors and spec sizes.
- Implement CameraFit.Polylines union-bounds behavior with token padding.
- Implement missing-token fallback via existing LSGlassPanel without crashing.
- Implement nested-scroll isolation.
- Keep the 9 sandbox stories compiling.
...
If any full-scope command is blocked by pre-existing issues, document the exact failure in the completion JSON.
...
- TC-1: LSMapTest.three_polylines_use_token_colors passes: best=EE7C2B, alt1=4D8470, alt2=6B7B8F
- TC-2: LSMapTest.annotations_render_at_spec_sizes passes: start=14/end=18/6/waypoint=12
- TC-3: LSMapTest.missing_token_shows_glass_panel_fallback passes: no MapView created
- TC-4: LSMapTest.network_unavailable_shows_glass_panel_fallback passes
- TC-5: LSMap.kt contains zero raw Color(0x literals in polyline/annotation code
- TC-6: LSMap.kt calls rememberNestedScrollInteropConnection for scroll isolation
- TC-7: LSMap.kt has no hardcoded mapbox:// URL outside GeneratedTokens references
- TC-8: LSMapStories.kt defines exactly 9 stories matching iOS parity manifest

Task file follows below. Execute directly against it and start editing now.
