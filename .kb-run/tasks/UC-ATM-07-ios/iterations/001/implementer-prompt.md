...
- Repo-current drift: the tree already contains a legacy Badge.swift that uses #Preview and SF Symbols. Treat it as reference only; do not mutate it unless the task scope explicitly allows it, and prefer the LS-prefixed deliverables required by the task contract.
- Current iOS story aggregation lives in ios/LaneShadow/Sandbox/LaneShadowStories.swift plus ios/LaneShadow/Sandbox/Stories/AtomsStories.swift.
...
- Write a concise evidence log to `/Users/justinrich/Projects/LaneShadow/.kb-run/tasks/UC-ATM-07-ios/iterations/001/evidence.md`.
- Write an evidence manifest JSON to `/Users/justinrich/Projects/LaneShadow/.kb-run/tasks/UC-ATM-07-ios/iterations/001/evidence-manifest.json` with:
  - `task_id`
  - `files_changed`
