...
- Respect `RULES.md` and keep scope limited to this task's allowed files.

## Objective

Implement `UC-MOL-02-android` end-to-end per the task file using RED -> GREEN -> REFACTOR. Produce the molecule code, tests, story registration, any required project generation change, and a concise learnings note at `ai-specs/UC-MOL-02-android/android-learnings.md` if you create or update learnings.

## Required Reading

1. `.spec/prds/v2/tasks/sprint-04-molecules-composite-patterns/UC-MOL-02-android-toolbar-navheader-molecules.md`
2. `RULES.md`
3. `~/Projects/brain/docs/ROOT-CONTEXT.md`
4. The task reading-list files referenced in the task markdown
5. Existing related atoms, stories, and molecule patterns in this worktree before editing

## Scope

Use the task file's `writeAllowed` / `writeProhibited` lists as the hard scope contract. If the task requires generated project outputs, edit the source-of-truth input (for example `ios/project.yml`) and generate the derived file rather than hand-editing generated outputs.
...
Run the task `RUNTIME_COMMANDS` and any targeted test commands named in the acceptance criteria. Include exact pass/fail outcomes in your completion report.

## Completion Contract

Before exiting:

1. Make a normal git commit in the task worktree.
2. Leave the worktree clean.
3. Write your final response to the configured output file.
4. Return JSON matching this shape exactly:
...
```json
{
  ...
  "failed_commands": [],
  "evidence_path": "path to primary evidence artifact or summary file",
  "evidence_manifest_path": "path to evidence manifest or supporting note",
  "summary": "what changed and current result",
  "files_changed": ["relative/path"],
  "verification_commands": ["command"],
  "acceptance_criteria_evidence": [
    {"id": "AC-1", "status": "met | not_met", "evidence": ["proof"]}
  ],
  "reviewer_considerations": ["risks or focus areas"],
  "notes": "include base sha, final commit sha, TDD RED/GREEN evidence, screenshot path if produced"
}
```

If you are blocked, do not guess. Return `status: blocked` with the concrete blocker, what you tried, and the narrowest user-actionable or orchestrator-actionable unblock options.
