...
// dotted_order segment: YYYYMMDDTHHMMSSmmmZ<uuid>  (millisecond precision)
...
function toNano(date) { return String(date.getTime() * 1_000_000) }
...
for (let attempt = 1; Date.now() < deadline && !found; attempt++) {
  ...
  try {
    ...
    if (r1.status === 200) {
      ...
      if (list.length >= 3) { found = { via: 'v2/traces/{id}/runs', count: list.length, types: list.map(r=>r.run_type) }; break }
    }
  } catch {}
