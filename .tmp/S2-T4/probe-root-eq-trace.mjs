...
// trace_id == root run id (LangSmith: a trace IS its root run)
const traceUuid = randomUUID()
const rootUuid = traceUuid           // <-- root span id == trace id
...
function dottedSegment(date, uuid) {
  ...
  return `${d.getUTCFullYear()}${pad(d.getUTCMonth()+1)}${pad(d.getUTCDate())}T${pad(d.getUTCHours())}${pad(d.getUTCMinutes())}${pad(d.getUTCSeconds())}${pad(d.getUTCMilliseconds(),3)}Z${uuid}`
}
...
const rootDotted = dottedSegment(rootTime, rootUuid)
...
const payload = { resourceSpans:[{ resource:{attributes:[{key:'service.name',value:{stringValue:'laneshadow-agent'}}]}, scopeSpans:[{scope:{name:'@mastra/observability'}, spans:[
  mk('1111111111111111','agent-run','chain',undefined,rootUuid,rootDotted,rootTime,new Date(rootTime.getTime()+500)),
  mk('2222222222222222','llm: claude','llm',rootUuid,modelUuid,modelDotted,modelTime,new Date(modelTime.getTime()+400)),
  mk('3333333333333333','tool: geocode','tool',rootUuid,toolUuid,toolDotted,toolTime,new Date(toolTime.getTime()+300)),
]}] }] }
...
for (let attempt = 1; Date.now() < deadline && !found; attempt++) {
  ...
  try {
    ...
    if (r1.status === 200) {
      ...
      if (list.length >= 3) { found = { via:'v2/traces/{id}/runs', count:list.length, types:list.map(r=>r.run_type), sample_id:list[0]?.id }; break }
    }
  } catch {}
  console.log(`attempt ${attempt}: not yet`)
}
